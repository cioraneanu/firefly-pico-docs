...
Ramble is configured on the **backend** through environment variables — all of them **optional**. If nothing is set, the Ramble button simply doesn't appear and Pico behaves exactly as before.
...
| `ASSISTANT_LLM_CONTEXT` | — | Extra instructions appended to the system prompt for every interpretation (e.g. `"Groceries from Lidl go to the Food category"`). |
...
services:
  firefly-pico:
    ...
    environment:
      ...
      # - ASSISTANT_LLM_CONTEXT=Groceries from Lidl go to the Food category
...
The endpoint above makes iOS Shortcuts a perfect companion: raise your wrist, say "Hey Siri, Pico", ramble about what you just paid, done. Two ready-made **signed** templates live in the repo under [`docs/siri-shortcuts`](https://github.com/cioraneanu/firefly-pico/tree/main/docs/siri-shortcuts) — import one, fill in your URL and token, and you're set. Or build them yourself:
