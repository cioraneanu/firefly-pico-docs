...
Use text mode when the client already has typed or dictated text. It only requires the Ramble language model to be [configured](/features/ramble/configuration#1-configure-transaction-interpretation).
...
Use audio mode to upload the recording itself. Pico stores it and uses the server-side transcription service before interpretation. [Audio transcription must be configured](/features/ramble/configuration#2-configure-audio-transcription-optional).
...
::tip
For a ready-made automation, continue with the [iOS Shortcuts guide](/features/ramble/ios).
::
